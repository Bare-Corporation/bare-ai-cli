...
for DB_ID in 2 3; do
  ...
  TABLES=$(curl -s "http://[INTERNAL_IP]/api/database/tables/database/${DB_ID}/" -H "Authorization: JWT ${TOKEN}")
  ...
  while IFS='|' read -r tid tname; do
    ...
    ROWS=$(curl -s "http://[INTERNAL_IP]/api/database/rows/table/${tid}/?size=10000" -H "Authorization: JWT ${TOKEN}")
