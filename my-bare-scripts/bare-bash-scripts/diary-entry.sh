...
### Key Learnings
- **[INTERNAL_SERVICE] 2.2.2 OSE** has no `/api/database/export/async/`, no `/api/schema/`
...
- Django multi-table inheritance in [INTERNAL_SERVICE]: `database_field` is parent, child tables use `field_ptr_id` FK back to parent
