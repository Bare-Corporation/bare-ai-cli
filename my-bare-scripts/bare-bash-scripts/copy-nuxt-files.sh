...
FILES=$(sudo docker exec bare-table-web-frontend-1 bash -c 'ls /[INTERNAL_SERVICE]/web-frontend/.output/public/_nuxt/*.js /[INTERNAL_SERVICE]/web-frontend/.output/public/_nuxt/*.js.map 2>/dev/null')
COUNT=0
for f in $FILES; do
  fname=$(basename "$f")
