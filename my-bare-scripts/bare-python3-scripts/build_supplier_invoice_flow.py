#!/usr/bin/env python3
"""
Build FreeAgent Supplier Invoice -> [INTERNAL_SERVICE] integration on NiFi canvas.
...
print('[*] Building FreeAgent Supplier Invoice -> [INTERNAL_SERVICE] Integration')
print()
...
procs['transform'] = create_processor(l5_id, 'NiFi Groovy Script: FreeAgent Supplier Bill to [INTERNAL_SERVICE] Transformer',
...
procs['upd_attr'] = create_processor(l5_id, 'UpdateAttribute - [INTERNAL_SERVICE] Write Config',
    'org.apache.nifi.processors.attributes.UpdateAttribute', 3500, 0,
    config={'schedulingStrategy': 'TIMER_DRIVEN',
            'properties': {
                'api.base_url': '#{[INTERNAL_SERVICE] API URL}',
                'api.target_table_id': '#{Base_Bare-Finance_SupplierInvoices_TableID}',
                'api.url.append_trailing_slash': 'true',
                'api.startingParameter': '?user_field_names=true',
                'authorization': 'Token #{NifiBareERP_PatToken_[INTERNAL_SERVICE]}',
                'invoke.http.method': 'POST',
                'http.header.Content-Type': 'application/json',
                'http.header.Accept': 'application/json'
            }})

procs['invoke_br'] = create_processor(l5_id, 'InvokeHTTP - [INTERNAL_SERVICE] Supplier Invoice Write',
...
procs['route_br'] = create_processor(l5_id, '[INTERNAL_SERVICE]_CentralRouter_HandleHTTP_Response',
